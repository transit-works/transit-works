...
impl Actor for WebSocketProxy {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        debug!(
            "WebSocket proxy started for city '{}' at port {}",
            self.city, self.port
        );
        ...
        let req = client.ws(ws_url).max_frame_size(MAX_PAYLOAD_SIZE);
        ...
        ctx.spawn(fut.into_actor(self).then(|res, act, ctx| {
            if let Some(connect_res) = res {
                ...
                let stream_future = stream.for_each(move |msg| {
                    match msg {
                        ...
                        Ok(Frame::Text(bytes)) => {
                            ...
                            if let Ok(text) = String::from_utf8(bytes.to_vec()) {
                                ...
                                if log_enabled!(log::Level::Debug) {
                                    debug!(
                                        "Received text from server ({}B), forwarding to client",
                                        bytes.len()
                                    );
                        ...
                        Ok(Frame::Binary(bin)) => {
                            debug!(
                                "Received binary from server ({}B), forwarding to client",
                                bin.len()
                            );
    ...
    fn handle(&mut self, msg: Result<ws::Message, ws::ProtocolError>, ctx: &mut Self::Context) {
        ...
        match msg {
            ...
            Ok(ws::Message::Text(text)) => {
                ...
                if let Some(sink) = &mut self.sink {
                    if let Err(e) =
                        futures::executor::block_on(sink.send(Message::Text(text.into())))
                    {
            ...
            Ok(ws::Message::Close(reason)) => {
                ...
                if let Some(sink) = &mut self.sink {
                    if let Err(e) =
                        futures::executor::block_on(sink.send(Message::Close(reason.clone())))
                    {
...
async fn websocket_proxy(
    ...
    debug!(
        "Handling WebSocket request to city '{}' at path '{}'",
        city, path
    );
            ...
            match res.body().limit(MAX_PAYLOAD_SIZE).await {
                ...
                Err(e) => {
                    ...
                    HttpResponse::InternalServerError()
                        .body(format!("Failed to get response body: {}", e))
                }
            }
        }
        Err(e) => {
