...
function MiniTable() {
  ...
  return (
    <div className="w-full bg-background-dk border-zinc-800 border rounded-2xl mt-2 p-2">
      {busData.map((item, index) => (
        <div key={index} className="flex items-center justify-between">
          <div className="flex-1 font-heading text-sm text-white">{item.label}</div>
          <div className="font-heading text-white text-sm pr-6 text-white">{item.count}</div>
          <div
